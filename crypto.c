...
bool crypto_verify_key(const char* encoded_key, const char* master_password) {
  ...
  if (sodium_hex2bin(key_with_salt, key_with_salt_len, encoded_key, strlen(encoded_key), NULL, NULL,
                     NULL) != 0) {
  ...
  bool result =
    (memcmp(derived_key, key_with_salt + crypto_pwhash_SALTBYTES, crypto_secretbox_KEYBYTES) == 0);
